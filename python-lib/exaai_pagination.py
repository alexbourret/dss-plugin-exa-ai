...
logger = SafeLogger("exa-ai pagination", ["password"])
DEFAULT_PAGE_SIZE = 50


class ExaAIPagination():
    ...
    def has_next_page(self, response, items_retrieved):
        if response is None:
            logger.info("ExaAIPagination:has_next_page:initialisation")
            self.page_offset = 0
            return True
    ...
    def get_paging_parameters(self, current_params):
        logger.info("ExaAIPagination:get_paging_parameters")
